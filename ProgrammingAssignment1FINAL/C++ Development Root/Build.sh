...
CheckVersion()
{
  ...
  if [[ "${Distribution,,}" = "ubuntu"  &&  "${Release,,}" = "22.04" ]]; then
      ...
      if [[ "${shall_I_upgrade,,}x" = "yesx"  ||  "${shall_I_upgrade,,}x" = "yx" ]]; then
        ...
        if [[ "${shall_I_upgrade,,}x" = "yesx"  ||  "${shall_I_upgrade,,}x" = "yx" ]]; then
          ...
          sudo /bin/bash -svx -- <<-EOF   # the "-" after the "<<" allows leading tabs (but not spaces), a quoted EOF would mean literal input, i.e., do not substitute parameters
				...
				apt -y update
				apt -y full-upgrade  # takes too long and sometimes doesn't work on the VCL, otherwise it's a good idea but not required

				apt -y install build-essential manpages-dev gdb ddd --install-suggests # in case "Tuffix" is not installed (e.g. WSL)

				apt -y install gcc-${RequiredGccVersion} g++-${RequiredGccVersion} --install-suggests
				...
				apt -y install clang-${RequiredClangVersion} clang-tools-${RequiredClangVersion} clang-${RequiredClangVersion}-doc libclang-common-${RequiredClangVersion}-dev libclang-${RequiredClangVersion}-dev libclang1-${RequiredClangVersion} clang-format-${RequiredClangVersion} clang-tidy-${RequiredClangVersion} python3-clang-${RequiredClangVersion} clangd-${RequiredClangVersion} --install-suggests
				apt -y install lldb-${RequiredClangVersion} lld-${RequiredClangVersion} --install-suggests
				apt -y install libc++-${RequiredClangVersion}-dev libc++abi-${RequiredClangVersion}-dev --install-suggests
				apt -y autoremove
  ...
  if [[ "${Distribution,,}" = "ubuntu"  &&  "${Release,,}" = "20.04" ]]; then
    echo -e "You're still using Ubuntu 20.04 (Focal Fossa), but we've moved on to Ubuntu 22.04 (Jammy Jellyfish) and updated compilers.\nIt may still \"work\" for awhile, but you should move up too.\n\n"
...
echo "${0##*/} version ${versionID} compiling in \"$PWD\" ..."
